 ...
 *
 * CRITIQUE ASSIGNMENTS SEAM (issue #21 AC3): the Critique Order section must
 * reflect the LOCKED critique assignments once the critique-randomizer slice
 * (#23) lands. That slice does not exist yet and the Contest model carries no
 * assignment data, so this port renders v12's no-assignments-yet state exactly:
 * a single sentence describing WHEN critiques happen (after each show vs. after
 * all shows), the order "randomly chosen, announced" at contest time. When #23
 * adds locked assignments to the model, render them here in place of / beneath
 * that sentence — that is the only change this document needs for full
 * integration; the rest of the agenda is fixed language.
 *
